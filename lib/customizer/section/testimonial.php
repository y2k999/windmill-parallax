...
<?php
	 ...
	 * @return (array)
	*/
	if(function_exists('__get_section_child_testimonial') === FALSE) :
	function __get_section_child_testimonial()
	{
		return array(
			'testimonial_general' => esc_html__('General Settings','windmill'),
			'testimonial_first' => esc_html__('First Testimonial','windmill'),
			'testimonial_second' => esc_html__('Second Testimonial','windmill'),
			'testimonial_third' => esc_html__('Third Testimonial','windmill'),
		);
	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		foreach(__get_section_child_testimonial() as $key => $value){
			/**
			 * [NOTE]
