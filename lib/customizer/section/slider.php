...
<?php
	 ...
	 * @return (array)
	*/
	if(function_exists('__get_section_child_slider') === FALSE) :
	function __get_section_child_slider()
	{
		return array(
			'slider_general' => esc_html__('General Settings','windmill'),
			'slider_first' => esc_html__('First Slider','windmill'),
			'slider_second' => esc_html__('Second Slider','windmill'),
			'slider_third' => esc_html__('Third Slider','windmill'),
		);
	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		foreach(__get_section_child_slider() as $key => $value){
			/**
			 * [NOTE]
