...
<?php
	 ...
	 * @return (array)
	*/
	if(function_exists('__get_section_child_price') === FALSE) :
	function __get_section_child_price()
	{
		return array(
			'price_general' => esc_html__('General Settings','windmill'),
			'price_first' => esc_html__('First Price','windmill'),
			'price_second' => esc_html__('Second Price','windmill'),
			'price_third' => esc_html__('Third Price','windmill'),
		);
	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		foreach(__get_section_child_price() as $key => $value){
			/**
			 * [NOTE]
