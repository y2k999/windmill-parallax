...
<?php
	 ...
	 * @return (array)
	*/
	if(function_exists('__get_section_child_service') === FALSE) :
	function __get_section_child_service()
	{
		return array(
			'service_general' => esc_html__('General Settings','windmill'),
			'service_first' => esc_html__('First Service','windmill'),
			'service_second' => esc_html__('Second Service','windmill'),
			'service_third' => esc_html__('Third Service','windmill'),
		);
	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		foreach(__get_section_child_service() as $key => $value){
			/**
			 * [NOTE]
