...
<?php
	 ...
	 * @return (array)
	*/
	if(function_exists('__get_section_child_team') === FALSE) :
	function __get_section_child_team()
	{
		return array(
			'team_general' => esc_html__('General Settings','windmill'),
			'team_first' => esc_html__('First Team','windmill'),
			'team_second' => esc_html__('Second Team','windmill'),
			'team_third' => esc_html__('Third Team','windmill'),
		);
	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		foreach(__get_section_child_team() as $key => $value){
			/**
			 * [NOTE]
