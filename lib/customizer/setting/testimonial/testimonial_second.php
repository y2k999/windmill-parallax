	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		*/
		...
		// description_second_testimonial
		$wp_customize->add_setting('setting_WP_description_second_testimonial',array(
			'default' => __get_customizer_default_child('description_second_testimonial'),
			'capability' => 'edit_theme_options',
			'sanitize_callback' => '__utility_sanitize_textarea'
		));
		$wp_customize->add_control('setting_WP_description_second_testimonial', array(
			'settings' => 'setting_WP_description_second_testimonial',
			'label' => esc_html__('Testimonial Description No.2','windmill'),
			'section' => 'section_WP_' . $_section,
			'type' => 'textarea',
		));

		// name_second_testimonial
		$wp_customize->add_setting('setting_WP_name_second_testimonial',array(
			'default' => __get_customizer_default_child('name_second_testimonial'),
			'capability' => 'edit_theme_options',
			'sanitize_callback' => '__utility_sanitize_text'
		));
		$wp_customize->add_control('setting_WP_name_second_testimonial',array(
			'settings' => 'setting_WP_name_second_testimonial',
			'label' => esc_html__('Testimonial Author No.2','windmill'),
			'section' => 'section_WP_' . $_section,
			'type' => 'text',
		));
