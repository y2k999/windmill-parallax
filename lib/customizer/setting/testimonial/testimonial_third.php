	...
	*/
	beans_add_smart_action('customize_register',function($wp_customize)
	{
		...
		*/
		...
		// description_third_testimonial
		$wp_customize->add_setting('setting_WP_description_third_testimonial',array(
			'default' => __get_customizer_default_child('description_third_testimonial'),
			'capability' => 'edit_theme_options',
			'sanitize_callback' => '__utility_sanitize_textarea'
		));
		$wp_customize->add_control('setting_WP_description_third_testimonial', array(
			'settings' => 'setting_WP_description_third_testimonial',
			'label' => esc_html__('Testimonial Description No.3','windmill'),
			'section' => 'section_WP_' . $_section,
			'type' => 'textarea',
		));

		// name_third_testimonial
		$wp_customize->add_setting('setting_WP_name_third_testimonial',array(
			'default' => __get_customizer_default_child('name_third_testimonial'),
			'capability' => 'edit_theme_options',
			'sanitize_callback' => '__utility_sanitize_text'
		));
		$wp_customize->add_control('setting_WP_name_third_testimonial',array(
			'settings' => 'setting_WP_name_third_testimonial',
			'label' => esc_html__('Testimonial Author No.3','windmill'),
			'section' => 'section_WP_' . $_section,
			'type' => 'text',
		));
